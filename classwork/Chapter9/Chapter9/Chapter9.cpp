...
void CStringDemp()
{
    ...
    //Normally use char* for C string
...
void CStringFunctions()
{
    ...
    strcpy(output, input);  // strcpy(target, source) :: Copies source to target
                            ...
                            // If output is exatly the length of input then no NULL is written 
                            // If ouput is smaller than input then it overwrites memory (security vuln)
    ...
    strncmp(output, input, MaxInputSize); // Compares 2 stings up to max len
    ...
    char* end;
    strtol("123abc", &end, 0); //Parses a stgring to a long, uing the base given
